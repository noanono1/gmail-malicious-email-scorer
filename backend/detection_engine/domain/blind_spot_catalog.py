...
# Tags whose presence means the HTML carries visual structure beyond
# plain-text wrapping (Gmail wraps even plain-text messages in <div>).
_STRUCTURAL_HTML_PATTERN = re.compile(
    r"<(?:a|form|img|table|iframe|style|input|button|select|textarea)\b",
    re.IGNORECASE,
)
...
def _has_meaningful_html(email: EmailData) -> bool:
    return bool(email.body_html) and bool(_STRUCTURAL_HTML_PATTERN.search(email.body_html))
...
STRUCTURAL: tuple[BlindSpot, ...] = (
    ...
    BlindSpot(
        ...
        risk_note="The email was analyzed as extracted content, not as a fully rendered visual page.",
        applies=_has_meaningful_html,
    ),
)
...
AUTHENTICATION_HEADERS_MISSING = BlindSpot(
    ...
    risk_note="SPF, DKIM, and DMARC results were not available in the email headers.",
)
...
_AUTHENTICATION_UNENFORCEABLE_RISK_NOTES: dict[tuple[str, str], str] = {
    ...
    ("dmarc", "none"):      "Sender domain has no DMARC policy — cannot verify the visible From domain is authenticated.",
...
URL_DESTINATION = BlindSpot(
    ...
    risk_note="URLs were detected, but destination pages were not opened or analyzed.",
)

SENDER_ADDRESS_UNPARSEABLE = BlindSpot(
    area=BlindSpotArea.SENDER_IDENTITY,
    reason="From address could not be parsed",
    risk_note="Sender-identity checks were skipped because the From address could not be parsed.",
)
...
LANGUAGE_ASSESSMENT_UNAVAILABLE = BlindSpot(
    ...
    risk_note="Social-engineering language patterns could not be assessed for this email.",
)
