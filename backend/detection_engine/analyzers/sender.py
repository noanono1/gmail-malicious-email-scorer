...
_extractor = tldextract.TLDExtract(suffix_list_urls=()) # No network calls, use static suffix list
...
# ---------------------------------------------------------------------------
# Brand intelligence — brand name → legitimate domains owned by that brand
# ---------------------------------------------------------------------------
...
_ALL_BRAND_DOMAINS: frozenset[str] = frozenset().union(*_BRAND_DOMAINS.values())
...
# ---------------------------------------------------------------------------
# Display name identity-token filtering
# ---------------------------------------------------------------------------

_NON_IDENTITY_TOKENS: frozenset[str] = frozenset({
    "bank", "security", "support", "helpdesk", "official",
    "team", "department", "dept", "corporate", "inc", "ltd",
    "foundation", "institute", "government", "federal",
...
def _is_brand_domain(domain: str) -> bool:
    if domain in _ALL_BRAND_DOMAINS:
        return True
    return any(domain.endswith(f".{legit}") for legit in _ALL_BRAND_DOMAINS)
...
def _extract_claimed_identity_tokens(display_name: str) -> list[str]:
    raw_tokens = re.split(r"[\s\-_.,;:!?|/\\@()\[\]<>\"']+", display_name)
    return [
        t.lower()
        for t in raw_tokens
        if len(t) >= 3 and t.lower() not in _NON_IDENTITY_TOKENS
    ]
...
class SenderAnalyzer(BaseAnalyzer):
    ...
    def analyze(self, email: EmailData) -> AnalysisOutput:
        ...
        self._check_display_name_impersonation(email, sender_domain, signals)
    ...
    def _check_cousin_domain(
        ...
        if _is_brand_domain(sender_domain):
            return

        segments = _sender_domain_segments(sender_domain)

        for segment in segments:
            normalized_segment = _normalize(segment.lower())
            for brand in _BRAND_DOMAINS:
                threshold = _max_typosquat_distance(brand)
                distance = _levenshtein(normalized_segment, brand)
                    ...
                    signals.append(
                        Signal(
                            ...
                            evidence=(
                                ...
                                f"resembles brand '{brand}'"
                            ),
                        )
                    )
                    return

    def _check_display_name_impersonation(
        ...
        identity_tokens = _extract_claimed_identity_tokens(display_name)
        if not identity_tokens:
            return

        for token in identity_tokens:
            normalized = _normalize(token)
            if normalized not in _BRAND_DOMAINS:
                continue

            claimed_brand = normalized
            brand_domains = _BRAND_DOMAINS[claimed_brand]

            registered = _registered_domain(sender_domain)
            if registered in brand_domains:
                return

            if any(s.id == "cousin_domain" for s in signals):
                return
            ...
            signals.append(
                Signal(
                    ...
                    confidence=0.8,
                    evidence=(
                        f"Display name claims '{claimed_brand}' "
