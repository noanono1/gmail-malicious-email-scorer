"""Unit tests for BodyContentAnalyzer — urgency, sensitive data requests, HTML forms."""
...
class TestUrgencyLanguage:
    def test_suspended_within_24_hours(self, analyzer: BodyContentAnalyzer):
    ...
    def test_immediate_action_required(self, analyzer: BodyContentAnalyzer):
    ...
    def test_time_sensitive(self, analyzer: BodyContentAnalyzer):
        email = _make_email(body_text="This is time-sensitive and must be handled today.")
        output = analyzer.analyze(email)
        assert any(s.id == "urgency_language" for s in output.signals)

    def test_respond_immediately(self, analyzer: BodyContentAnalyzer):
        email = _make_email(body_text="Please respond immediately.")
        output = analyzer.analyze(email)
        assert any(s.id == "urgency_language" for s in output.signals)

    def test_service_interruption(self, analyzer: BodyContentAnalyzer):
        email = _make_email(body_text="To avoid service interruption, update now.")
        output = analyzer.analyze(email)
        assert any(s.id == "urgency_language" for s in output.signals)

    def test_subject_scanned_for_urgency(self, analyzer: BodyContentAnalyzer):
        email = _make_email(
    ...
    def test_benign_deadline_not_flagged(self, analyzer: BodyContentAnalyzer):
    ...
    def test_normal_shipping_not_flagged(self, analyzer: BodyContentAnalyzer):
        email = _make_email(body_text="Your order has shipped! Arriving Tuesday.")
        output = analyzer.analyze(email)
        assert not [s for s in output.signals if s.id == "urgency_language"]

    def test_multiple_matches_increase_confidence(self, analyzer: BodyContentAnalyzer):
        email = _make_email(
            body_text=(
    ...
    def test_single_match_has_lower_confidence(self, analyzer: BodyContentAnalyzer):
        email = _make_email(body_text="This is time-sensitive.")
        output = analyzer.analyze(email)
        signal = [s for s in output.signals if s.id == "urgency_language"][0]
        assert signal.confidence == pytest.approx(0.65, abs=0.01)

    def test_only_one_urgency_signal_per_email(self, analyzer: BodyContentAnalyzer):
        email = _make_email(
...
class TestSensitiveDataRequest:
    def test_verify_identity(self, analyzer: BodyContentAnalyzer):
    ...
    def test_update_payment(self, analyzer: BodyContentAnalyzer):
    ...
    def test_bank_account_details(self, analyzer: BodyContentAnalyzer):
        email = _make_email(body_text="Please send your bank account details.")
        output = analyzer.analyze(email)
        assert any(s.id == "sensitive_data_request" for s in output.signals)

    def test_verify_password(self, analyzer: BodyContentAnalyzer):
        email = _make_email(body_text="You need to verify your password.")
        output = analyzer.analyze(email)
        assert any(s.id == "sensitive_data_request" for s in output.signals)

    def test_social_security(self, analyzer: BodyContentAnalyzer):
        email = _make_email(body_text="Please provide your social security number.")
        output = analyzer.analyze(email)
        assert any(s.id == "sensitive_data_request" for s in output.signals)

    def test_normal_account_reference_not_flagged(self, analyzer: BodyContentAnalyzer):
    ...
    def test_password_reset_notification_not_flagged(self, analyzer: BodyContentAnalyzer):
        email = _make_email(
    ...
    def test_only_one_sensitive_signal_per_email(self, analyzer: BodyContentAnalyzer):
        email = _make_email(
...
class TestRealFixtures:
    def test_mass_phishing_urgency_and_sensitive(self, analyzer: BodyContentAnalyzer):
    ...
    def test_spear_phish_urgency_and_sensitive(self, analyzer: BodyContentAnalyzer):
    ...
    def test_bec_has_urgency(self, analyzer: BodyContentAnalyzer):
    ...
    def test_malware_has_urgency(self, analyzer: BodyContentAnalyzer):
